...
module.exports = {
    name: 'setgreeting',
    description: 'Used for updating or setting your greeting. Use delgreeting to remove greeting entirely. Type = {cat or file}',
    ...
    execute({message, args}) {
        return new Promise(async(resolve,reject) => {
            ...
            if (isFile && isCategory) {
                if (!type) {return reject({ userMess: `"${greeting}" is both a file and a category name. Please specify the type. For example: "setgreeting!${greeting} <type>", type = {cat or file}`});}
            ...
            try {
                ...
                if (rtnMess === 'Success') {
                    let greeting_type_string = greeting_type == gd.GREETING_TYPE.FILE ? "file" : greeting_type == gd.GREETING_TYPE.CATEGORY ? "category" : "undefined";
                    return resolve({userMess: `Your greeting has been set to "${greeting}" with a type of ${greeting_type_string}!`});
                } else {
                    return reject({
