...
import * as tracksSchemas from "../schemas/tracksSchemas";
...
export const getTrackDownloadById = async (req: Request, res: Response) => {
  const { id } = i32IdSchema.parse(req.params);
  const track = await tracksService.getTrackById(id);
...
export const getTrackStreamById = async (req: Request, res: Response) => {
  const { id } = i32IdSchema.parse(req.params);
  const track = await tracksService.getTrackById(id);
  const file = await tracksService.getTrackFile(track);
...
export const postTrack = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (metadata.format.duration > config.maxTrackDuration) {
      throw Boom.badRequest(`Track duration exceeds limit of ${config.maxTrackDuration} seconds`);
    }
...
export const restoreTrack = async (req: Request, res: Response) => {
  const { id } = i32IdSchema.parse(req.params);
