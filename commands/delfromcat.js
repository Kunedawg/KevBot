...
const { categoryDict } = require('../globaldata.js');
    ...
    execute({message, args}) {
        return new Promise(async(resolve,reject) => {
            ...
            try {
                ...
                if (![Number(rtnDiscordId), Number(config.masterDiscordId)].includes(Number(discordId))) { return reject({userMess: `You did not create the category "${category}" so you may not remove audio from it!`}); }
            } catch (err) {
                return reject(err);               
            }        
